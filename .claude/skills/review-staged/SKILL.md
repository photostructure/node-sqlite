---
name: review-staged
description: Review git staged changes for concrete bugs, regressions, and missing validation before committing.
---
...
- Anything in `src/upstream/` — those files are auto-synced from Node.js and must not be modified (flag if the diff touches them)
...
- **Exploration**: When more than three files need review, or the diff spans both C++ and TypeScript layers, spawn explorer subagents (one per file/area) to gather findings
- **Validation**: Before reporting ANY issue, spawn a subagent to verify it — have it trace the full call chain (TS → N-API → C++ → SQLite C API), search for guards/handlers you might have missed, and check upstream Node.js behavior. If the subagent can't confirm the issue, discard it.
...
- **Refinement**: Are all the staged diffs a single coherent "story" or "theme"? If not, recommend to the user how the diffs should be split up before committing.
...
## Post-review commit flow (MANDATORY)

Do NOT commit directly after the review. Follow these steps in order:

1. List the files (and line ranges, if partial) you want to `git add`.
2. Compose a Conventional Commits message — `<type>(<scope>): <summary>` (under 50 chars on the first line; body explains the *why*) — and use `ask the user directly` to let the user edit it before committing.
