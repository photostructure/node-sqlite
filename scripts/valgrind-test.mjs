...
function runBasicOperations() {
  ...
  // Create table
  memDb.exec("CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT, value REAL)");
...
function runFileOperations() {
  ...
  try {
    ...
    // Prepare statements
    const insertUser = fileDb.prepare("INSERT INTO users (name, email) VALUES (?, ?)");
    ...
    if (count.count !== 40) throw new Error(`Expected 40 users, got ${count.count}`);
    fileDb2.close();
...
function runUserDefinedFunctions() {
  ...
  const doubled = db.prepare("SELECT double(n) as result FROM numbers WHERE n = 5").get();
  if (doubled.result !== 10) throw new Error(`Expected 10, got ${doubled.result}`);
