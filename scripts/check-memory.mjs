...
async function runJavaScriptMemoryTests() {
...
async function runValgrindTests() {
  if (platform() !== "linux") {
...
async function runASANTests() {
  if (platform() !== "linux") {
    log("\n=== AddressSanitizer Tests (skipped - Linux only) ===", colors.yellow);
    return 0;
  }
  ...
  const env = {
    ...process.env,
    LD_PRELOAD: asanLib,
    ASAN_OPTIONS: "detect_leaks=1:halt_on_error=0:print_stats=1:suppressions=" + join(projectRoot, ".asan-suppressions.txt"),
  ...
  const hasOurErrors = asanText.includes("node_sqlite.node") || 
...
async function main() {
  ...
  if (process.env.ENABLE_ASAN === "1") {
    ...
    log("\n=== AddressSanitizer Tests (skipped - set ENABLE_ASAN=1 to run) ===", colors.yellow);
  }
