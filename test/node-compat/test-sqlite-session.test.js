 ...
 * Adapted from: test-sqlite-session.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-session.test.js
 *
...
test("creating and applying a changeset", (t) => {
  ...
  const insert = databaseFrom.prepare(
...
test("database.createSession() - use table option to track specific table", (t) => {
  ...
  const insert1 = database1.prepare(
    "INSERT INTO data1 (key, value) VALUES (?, ?)",
  );
  insert1.run(1, "hello");
  insert1.run(2, "world");
  const insert2 = database1.prepare(
    "INSERT INTO data2 (key, value) VALUES (?, ?)",
  );
  insert2.run(1, "hello");
  insert2.run(2, "world");
  ...
  deepStrictEqual(t)(
    ...
    database2.prepare(select1).all(),
  ); // data1 table should be equal
...
suite("conflict resolution", () => {
  ...
  test("database.applyChangeset() - SQLITE_CHANGESET_CONSTRAINT conflict", (t) => {
    ...
    deepStrictEqual(t)(database2.prepare("SELECT key, value from data").all(), [
      { key: 2, value: "hello" },
    ]);
  });
...
test(
  "database.applyChangeset() - malformed changeset returns SQLITE_CORRUPT",
  {
    skip: process.config.variables.node_shared_sqlite
      ? "requires the bundled SQLite session fix"
      : false,
  },
  (t) => {
    const database = new DatabaseSync(":memory:");
...
test("session.patchset()", (t) => {
  ...
  database.exec(
    "INSERT INTO data VALUES ('1', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.')",
  );
...
test.skip("concurrent applyChangeset with workers" /* Worker thread changeset serialization issue */, async (t) => {
  ...
  function modeToString(mode) {
    if (mode === constants.SQLITE_CHANGESET_ABORT)
      return "SQLITE_CHANGESET_ABORT";
    if (mode === constants.SQLITE_CHANGESET_OMIT)
