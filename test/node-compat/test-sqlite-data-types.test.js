 ...
 * Adapted from: test-sqlite-data-types.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-data-types.test.js
 *
...
suite("data binding and mapping", () => {
  test("supported data types", (t) => {
    ...
    t.assert.deepStrictEqual(query.get(6), {
      __proto__: null,
      key: 6,
      int: null,
      double: null,
  ...
  test("undefined is bound as NULL", (t) => {
    ...
    t.assert.deepStrictEqual(stmt.run({ k: 1, v: undefined }), {
      changes: 1,
      lastInsertRowid: 1,
    });
    t.assert.deepStrictEqual(stmt.run({ k: 2 }), {
      changes: 1,
      lastInsertRowid: 2,
    });
    t.assert.deepStrictEqual(query.get(1), {
    ...
    t.assert.deepStrictEqual(db.prepare("SELECT ? AS a").get(undefined), {
      __proto__: null,
      a: null,
    });
  });
