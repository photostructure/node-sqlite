 ...
 * Adapted from: test-sqlite.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite.test.js
 *
...
suite.skip(
  "accessing the node:sqlite module" /* Tests Node.js built-in module loading */,
  () => {
    ...
    test.skip("can be disabled with --no-experimental-sqlite flag" /* Tests Node.js CLI flag */, async (t) => {
      const { stdout, stderr, code, signal } = await spawnPromisified(
        process.execPath,
        ["--no-experimental-sqlite", "-e", 'require("@photostructure/sqlite")'],
      );
...
test("in-memory databases are supported", (t) => {
  ...
  t.assert.deepStrictEqual(db1.prepare("SELECT * FROM data").all(), [
    { __proto__: null, key: 1 },
  ]);
  t.assert.deepStrictEqual(db2.prepare("SELECT * FROM data").all(), [
    { __proto__: null, key: 1 },
  ]);
});
...
test("PRAGMAs are supported", (t) => {
  ...
  t.assert.deepStrictEqual(db.prepare("PRAGMA journal_mode = WAL").get(), {
    __proto__: null,
    journal_mode: "wal",
  });
  t.assert.deepStrictEqual(db.prepare("PRAGMA journal_mode").get(), {
    __proto__: null,
    journal_mode: "wal",
  });
});
...
suite("URI query params", () => {
  ...
  test("query params are supported with URL objects", (t) => {
    ...
    t.assert.deepStrictEqual(readOnlyDB.prepare("SELECT * FROM data").all(), [
      { __proto__: null, key: 1 },
    ]);
    t.assert.throws(
      () => {
...
suite("SQL APIs enabled at build time", () => {
  ...
  test("fts3 parenthesis is enabled", (t) => {
    ...
    t.assert.deepStrictEqual(
      db
        .prepare(
          "SELECT * FROM t1 WHERE content MATCH '(groupedterm1 OR groupedterm2) OR hello world'",
        )
        .all(),
      [{ __proto__: null, content: "hello world" }],
    );
  ...
  test("rbu is enabled", (t) => {
    ...
    t.assert.deepStrictEqual(
      db
        .prepare(
          "SELECT sqlite_compileoption_used('SQLITE_ENABLE_RBU') as rbu_enabled;",
        )
        .get(),
  ...
  test("geopoly is enabled", (t) => {
    ...
    t.assert.deepStrictEqual(
      db
        .prepare(
          "SELECT rowid FROM t1 WHERE geopoly_contains_point(_shape, 0, 0)",
        )
        .get(),
