 ...
 * Adapted from: test-sqlite-aggregate-function.mjs
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-aggregate-function.test.mjs
 *
...
describe("varargs", () => {
  test("supports variable number of arguments when true", (t) => {
    ...
    const result = db
      .prepare("SELECT sum_int(value, 1, 2, 3) as total FROM data")
      .get();
...
describe("result", () => {
  ...
  test("executes once when options.inverse is not present", (t) => {
    ...
    const result = db
      .prepare("SELECT sum_int(value) as result FROM data")
      .get();
