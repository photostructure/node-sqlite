 ...
 * Adapted from: test-sqlite-authz.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-authz.test.js
 *
...
suite("DatabaseSync.prototype.setAuthorizer()", () => {
  ...
  it("receives correct parameters for SELECT operations", (t) => {
    ...
    assert.deepStrictEqual(callArguments, [
...
suite("authorizer callback reentrancy", () => {
  const expectedError =
    "ERR_INVALID_STATE: database cannot be accessed " +
  ...
  it("rejects methods the callback depth guard already covers", () => {
    ...
    assert.deepStrictEqual(runInAuthorizer(db, cases), {
      ...
      deserialize:
        "ERR_INVALID_STATE: database cannot be deserialized " +
        "while in a callback",
    });
  });
  ...
  it("allows iterator methods on a drained iterator", () => {
    ...
    const cases = {
      next: () => {
        done.next = iter.next().done;
      },
      return: () => {
        done.return = iter.return().done;
      },
    };
  ...
  it("allows access again after the authorizer returns", () => {
    ...
    db.setAuthorizer(() => constants.SQLITE_OK);
    assert.deepStrictEqual(db.prepare("SELECT 1 AS v").get(), {
      __proto__: null,
      v: 1,
    });
  });
