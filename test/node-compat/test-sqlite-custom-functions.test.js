 ...
 * Adapted from: test-sqlite-custom-functions.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-custom-functions.test.js
 *
...
suite("DatabaseSync.prototype.function()", () => {
  ...
  suite("return types", () => {
    test("supported return types", () => {
      ...
      db.function("retNumber", () => {
        return 3;
      });
      db.function("retString", () => {
        return "foo";
      });
      db.function("retBigInt", () => {
        return 5n;
      });
      db.function("retUint8Array", () => {
  ...
  suite("handles conflicting errors from SQLite and JavaScript", () => {
    test("throws if value cannot fit in a number", () => {
      ...
      assert.throws(
        () => {
          db.exec(
            `UPDATE test SET data = CUSTOM(${Number.MAX_SAFE_INTEGER + 1})`,
          );
        },
        {
          code: "ERR_OUT_OF_RANGE",
