 ...
 * Adapted from: test-sqlite-config.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-config.test.js
 *
...
function checkDefensiveMode(db) {
  ...
  switch (journalMode()) {
    case "memory":
      return true; // journal_mode unchanged, defensive mode must be active
    case "off":
      return false; // journal_mode now 'off', so defensive mode not active
    default:
      throw new Error("unexpected journal_mode");
  }
}
