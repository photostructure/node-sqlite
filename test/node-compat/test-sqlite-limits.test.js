 ...
 * Adapted from: test-sqlite-limits.js
 * Source: https://github.com/nodejs/node
 *
 * Run with: node --test test-sqlite-limits.test.js
 *
...
suite("DatabaseSync limits", () => {
  test("limits object has expected properties with positive values", (t) => {
    ...
    for (const prop of expectedProperties) {
      t.assert.strictEqual(
        typeof db.limits[prop],
        "number",
        `${prop} should be a number`,
      );
      t.assert.ok(db.limits[prop] > 0, `${prop} should be positive`);
    }
  });
  ...
  test("throws when exceeding column limit", (t) => {
    ...
    t.assert.throws(
      () => {
        db.exec(
          "CREATE TABLE t2 (c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11)",
        );
      },
      {
