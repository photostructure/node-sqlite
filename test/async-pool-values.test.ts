...
describe("DatabasePool values and rows", () => {
  ...
  test("binds JavaScript Numbers as REAL and BigInts as INTEGER", async () => {
    const pool = await DatabasePool.open(":memory:", { authorizer: "none" });
    try {
      await expect(
        pool.get("SELECT typeof(?) AS number_type, typeof(?) AS bigint_type", [
          1,
          1n,
        ]),
      ).resolves.toEqual({ number_type: "real", bigint_type: "integer" });
    } finally {
      await pool.close();
    }
  });
