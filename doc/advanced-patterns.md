...
db.prepare('UPDATE document SET content = ? WHERE id = ?').run('New content', 1);
...
const writeDb = new DatabaseSync(
  'file:data.db?mode=rwc&cache=private&psow=0'
);
...
console.log(`Rate: ${(data.length / duration * 1000).toFixed(0)} records/second`);
```
