...
UserDefinedFunction::UserDefinedFunction(Napi::Env env, 
                                       Napi::Function fn, 
                                       DatabaseSync* db, 
                                       bool use_bigint_args)
    : env_(env), fn_(Napi::Persistent(fn)), db_(db), use_bigint_args_(use_bigint_args) {
...
void UserDefinedFunction::xFunc(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
  ...
  try {
    ...
    sqlite3_result_error(ctx, error_msg.c_str(), static_cast<int>(error_msg.length()));
...
Napi::Value UserDefinedFunction::SqliteValueToJS(sqlite3_value* value) {
  switch (sqlite3_value_type(value)) {
    case SQLITE_INTEGER: {
      ...
      if (use_bigint_args_) {
        ...
        if (int_val < -0x1FFFFFFFFFFFFF || int_val > 0x1FFFFFFFFFFFFF) {
          ...
          std::string error_msg = "Value is too large to be represented as a JavaScript number: " + std::to_string(int_val);
    ...
    case SQLITE_BLOB: {
      ...
      return Napi::Buffer<uint8_t>::Copy(env_, 
...
void UserDefinedFunction::JSValueToSqliteResult(sqlite3_context* ctx, Napi::Value value) {
  if (value.IsNull() || value.IsUndefined()) {
    ...
    if (lossless) {
      ...
      sqlite3_result_text(ctx, bigint_str.c_str(), static_cast<int>(bigint_str.length()), SQLITE_TRANSIENT);
    }
    ...
    if (std::floor(num_val) == num_val && 
        num_val >= INT64_MIN && num_val <= INT64_MAX) {
    ...
    sqlite3_result_text(ctx, str_val.c_str(), static_cast<int>(str_val.length()), SQLITE_TRANSIENT);
  } else if (value.IsBuffer()) {
    Napi::Buffer<uint8_t> buffer = value.As<Napi::Buffer<uint8_t>>();
    sqlite3_result_blob(ctx, buffer.Data(), static_cast<int>(buffer.Length()), SQLITE_TRANSIENT);
  } else {
