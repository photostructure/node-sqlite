...
class Environment {
 public:
  static Environment* GetCurrent(Napi::Env env) {
    ...
    static thread_local std::unordered_map<napi_env, std::unique_ptr<Environment>> instances;
  ...
  Napi::String timeout_string() const { return Napi::String::New(env_, "timeout"); }
  Napi::String backup_string() const { return Napi::String::New(env_, "backup"); }
  Napi::String constants_string() const { return Napi::String::New(env_, "constants"); }
...
template<typename T>
void SetProtoMethod(napi_env env, Napi::Function constructor, 
