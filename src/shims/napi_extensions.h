...
class LocalVector {
 public:
  ...
  void push_back(Napi::Value value) {
...
template<typename T>
T* ASSIGN_OR_RETURN_UNWRAP(Napi::Value value) {
  if (!value.IsObject()) return nullptr;
  return T::Unwrap(value.As<Napi::Object>());
}

// Memory info macros (no-op for now)
#define SET_MEMORY_INFO_NAME(tracker, name) do { } while (0)
#define SET_SELF_SIZE(tracker, size) do { } while (0)
